...
/// @notice Burning can only be performed by accounts with the operator role.
/// @notice Transferability is disabled.
contract EDUNodeKey is IERC721, IERC721Burnable, ERC721Metadata, ERC721Mintable, ERC721Deliverable, TokenRecovery, ForwarderRegistryContext {
    ...
    /// @dev Emits a {Transfer} event with `to` set to the zero address for each token.
    /// @param from The current token owner.
