...
contract EDuCoinMerkleClaim is Pause, TokenRecovery, ForwarderRegistryContext {
    ...
    event PayoutClaimed(bytes32 indexed root, address indexed recipient, uint256 amount, uint256 treeCounter);

    /// @notice Thrown when trying to claim the same leaf more than once.
    /// @param recipient The recipient of the claim.
    /// @param amount The encoded claim data for the claim.
    /// @param treeCounter The treeCounter as when the claim was made.
    error AlreadyClaimed(address recipient, uint256 amount, uint256 treeCounter);

    /// @notice Thrown when a proof cannot be verified.
    /// @param recipient The recipient of the claim.
    /// @param amount uint256 amount of the ERC20 payout
    /// @param treeCounter The treeCounter as when the claim was made.
    error InvalidProof(address recipient, uint256 amount, uint256 treeCounter);
    ...
    function claimPayout(address recipient, uint256 amount, bytes32[] calldata proof, bytes calldata signature, uint256 expireAt) external {
        ...
        bytes32 leaf = keccak256(abi.encodePacked(recipient, amount, currentTreeCounter));

        if (claimed[leaf]) revert AlreadyClaimed(recipient, amount, currentTreeCounter);
        if (!proof.verifyCalldata(currentRoot, leaf)) revert InvalidProof(recipient, amount, currentTreeCounter);
